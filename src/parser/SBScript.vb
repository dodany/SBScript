...
Module MyParser
    ...
    Private Enum ProductionIndex
        ...
        [Definelist3] = 14                        ' <DefineList> ::= 
        [Incluye_Incluye_File] = 15               ' <Incluye> ::= INCLUYE FILE <nl>
        [Define_Define] = 16                      ' <Define> ::= DEFINE <DefineValue> <nl>
        [Definevalue_Num] = 17                    ' <DefineValue> ::= NUM
        [Definevalue_Str] = 18                    ' <DefineValue> ::= STR
        [Definevalue_Float] = 19                  ' <DefineValue> ::= FLOAT
        [Globaldec] = 20                          ' <Global Dec> ::= <Var Dec> <Global Dec>
        [Globaldec2] = 21                         ' <Global Dec> ::= <Method Dec> <Global Dec>
        [Globaldec3] = 22                         ' <Global Dec> ::= <Var Dec>
        [Globaldec4] = 23                         ' <Global Dec> ::= <Method Dec>
        [Vardec_Eq_Semi] = 24                     ' <Var Dec> ::= <Type> <List ID> '=' <Expression> ';' <nl Opt>
        [Vardec_Semi] = 25                        ' <Var Dec> ::= <Type> <List ID> ';' <nl Opt>
        [Listid_Id_Comma] = 26                    ' <List ID> ::= ID ',' <List ID>
        [Listid_Id] = 27                          ' <List ID> ::= ID
        [Type_Number] = 28                        ' <Type> ::= NUMBER
        [Type_Boolean] = 29                       ' <Type> ::= BOOLEAN
        [Type_String] = 30                        ' <Type> ::= STRING
        [Methoddec_Void_Id_Lparen_Rparen_Lbrace_Rbrace] = 31 ' <Method Dec> ::= VOID ID '(' <Arg List> ')' '{' <Instructions> '}' <nl Opt>
        [Methoddec_Id_Lparen_Rparen_Lbrace_Rbrace] = 32 ' <Method Dec> ::= <Type> ID '(' <Arg List> ')' '{' <Instructions> '}' <nl Opt>
        [Methoddec_Id_Lparen_Rparen_Lbrace_Rbrace2] = 33 ' <Method Dec> ::= ID '(' <Arg List> ')' '{' <Instructions> '}' <nl Opt>
        [Arglist_Id_Comma] = 34                   ' <Arg List> ::= <Type> ID ',' <nl Opt> <Arg List>
        [Arglist_Id] = 35                         ' <Arg List> ::= <Type> ID
        [Arglist] = 36                            ' <Arg List> ::= 
        [Instructions] = 37                       ' <Instructions> ::= <Instructions> <Instruction>
        [Instructions2] = 38                      ' <Instructions> ::= <Instruction>
        [Instruction] = 39                        ' <Instruction> ::= <Var Dec>
        [Instruction2] = 40                       ' <Instruction> ::= <Assign>
        [Instruction3] = 41                       ' <Instruction> ::= <Method Call>
        [Instruction4] = 42                       ' <Instruction> ::= <Return>
        [Instruction5] = 43                       ' <Instruction> ::= <If Stmt>
        [Instruction6] = 44                       ' <Instruction> ::= <Select Stmt>
        [Instruction7] = 45                       ' <Instruction> ::= <Para Stmt>
        [Instruction8] = 46                       ' <Instruction> ::= <Hasta Stmt>
        [Instruction9] = 47                       ' <Instruction> ::= <Mientras Stmt>
        [Instruction_Detener_Semi] = 48           ' <Instruction> ::= DETENER ';' <nl Opt>
        [Instruction_Continuar_Semi] = 49         ' <Instruction> ::= CONTINUAR ';' <nl Opt>
        [Instruction10] = 50                      ' <Instruction> ::= <nl Opt>
        [Assign_Id_Eq_Semi] = 51                  ' <Assign> ::= ID '=' <Expression> ';'
        [Methodcall_Mostrar_Lparen_Rparen_Semi] = 52 ' <Method Call> ::= MOSTRAR '(' <Param List> ')' ';' <nl Opt>
        [Methodcall_Dibujarast_Lparen_Id_Rparen_Semi] = 53 ' <Method Call> ::= DIBUJARAST '(' ID ')' ';' <nl Opt>
        [Methodcall_Dibujarexp_Lparen_Rparen_Semi] = 54 ' <Method Call> ::= DIBUJAREXP '(' <Expression> ')' ';' <nl Opt>
        [Methodcall_Id_Lparen_Rparen_Semi] = 55   ' <Method Call> ::= ID '(' <Param List> ')' ';' <nl Opt>
        [Return_Return_Semi] = 56                 ' <Return> ::= RETURN <Expression> ';' <nl Opt>
        [Return_Return_Semi2] = 57                ' <Return> ::= RETURN ';' <nl Opt>
        [Ifstmt_Si_Lparen_Rparen_Lbrace_Rbrace_Sino_Lbrace_Rbrace] = 58 ' <If Stmt> ::= SI '(' <Expression> ')' <nl Opt> '{' <Instructions> '}' <nl Opt> SINO <nl Opt> '{' <Instructions> '}' <nl Opt>
        [Ifstmt_Si_Lparen_Rparen_Lbrace_Rbrace] = 59 ' <If Stmt> ::= SI '(' <Expression> ')' <nl Opt> '{' <Instructions> '}' <nl Opt>
        [Selectstmt_Selecciona_Lparen_Rparen] = 60 ' <Select Stmt> ::= SELECCIONA '(' <Expression> ')' <nl Opt> <Case List> <Select Def>
        [Caselist] = 61                           ' <Case List> ::= <Case> <Case List>
        [Caselist2] = 62                          ' <Case List> ::= <Case>
        [Case_Colon_Lbrace_Rbrace] = 63           ' <Case> ::= <Select Value> ':' '{' <Instructions> '}' <nl Opt>
        [Selectdef_Defecto_Colon_Lbrace_Rbrace] = 64 ' <Select Def> ::= DEFECTO ':' '{' <Instructions> '}' <nl Opt>
        [Selectdef] = 65                          ' <Select Def> ::= 
        [Parastmt_Para_Lparen_Number_Id_Eq_Semi_Semi_Rparen_Lbrace_Rbrace] = 66 ' <Para Stmt> ::= PARA '(' NUMBER ID '=' <Expression> ';' <Expression> ';' <Para Op> ')' '{' <Instructions> '}' <nl Opt>
        [Paraop_Plusplus] = 67                    ' <Para Op> ::= '++'
        [Paraop_Minusminus] = 68                  ' <Para Op> ::= '--'
        [Hastastmt_Hasta_Lparen_Rparen_Lbrace_Rbrace] = 69 ' <Hasta Stmt> ::= HASTA '(' <Expression> ')' '{' <Instructions> '}' <nl Opt>
        [Mientrasstmt_Mientras_Lparen_Rparen_Lbrace_Rbrace] = 70 ' <Mientras Stmt> ::= MIENTRAS '(' <Expression> ')' '{' <Instructions> '}' <nl Opt>
        [Selectvalue_Num] = 71                    ' <Select Value> ::= NUM
        [Selectvalue_Float] = 72                  ' <Select Value> ::= FLOAT
        [Selectvalue_Str] = 73                    ' <Select Value> ::= STR
        [Expression_Gt] = 74                      ' <Expression> ::= <Expression> '>' <Or Exp>
        [Expression_Lt] = 75                      ' <Expression> ::= <Expression> '<' <Or Exp>
        [Expression_Lteq] = 76                    ' <Expression> ::= <Expression> '<=' <Or Exp>
        [Expression_Gteq] = 77                    ' <Expression> ::= <Expression> '>=' <Or Exp>
        [Expression_Eqeq] = 78                    ' <Expression> ::= <Expression> '==' <Or Exp>
        [Expression_Exclameq] = 79                ' <Expression> ::= <Expression> '!=' <Or Exp>
        [Expression_Tilde] = 80                   ' <Expression> ::= <Expression> '~' <Or Exp>
        [Expression] = 81                         ' <Expression> ::= <Or Exp>
        [Orexp_Pipepipe] = 82                     ' <Or Exp> ::= <Or Exp> '||' <Xor Exp>
        [Orexp] = 83                              ' <Or Exp> ::= <Xor Exp>
        [Xorexp_Exclamamp] = 84                   ' <Xor Exp> ::= <Xor Exp> '!&' <And Exp>
        [Xorexp] = 85                             ' <Xor Exp> ::= <And Exp>
        [Andexp_Ampamp] = 86                      ' <And Exp> ::= <And Exp> '&&' <Not Exp>
        [Andexp] = 87                             ' <And Exp> ::= <Not Exp>
        [Notexp_Exclam] = 88                      ' <Not Exp> ::= '!' <Not Exp>
        [Notexp] = 89                             ' <Not Exp> ::= <Add Exp>
        [Addexp_Plus] = 90                        ' <Add Exp> ::= <Add Exp> '+' <Mult Exp>
        [Addexp_Minus] = 91                       ' <Add Exp> ::= <Add Exp> '-' <Mult Exp>
        [Addexp] = 92                             ' <Add Exp> ::= <Mult Exp>
        [Multexp_Times] = 93                      ' <Mult Exp> ::= <Mult Exp> '*' <Pow Exp>
        [Multexp_Div] = 94                        ' <Mult Exp> ::= <Mult Exp> '/' <Pow Exp>
        [Multexp_Percent] = 95                    ' <Mult Exp> ::= <Mult Exp> '%' <Pow Exp>
        [Multexp] = 96                            ' <Mult Exp> ::= <Pow Exp>
        [Powexp_Caret] = 97                       ' <Pow Exp> ::= <Pow Exp> '^' <Negate Exp>
        [Powexp] = 98                             ' <Pow Exp> ::= <Negate Exp>
        [Negateexp_Minus] = 99                    ' <Negate Exp> ::= '-' <Value>
        [Negateexp] = 100                         ' <Negate Exp> ::= <Value>
        [Value_Id_Lparen_Rparen] = 101            ' <Value> ::= ID '(' <Param List> ')'
        [Value_Id] = 102                          ' <Value> ::= ID
        [Value_Lparen_Rparen] = 103               ' <Value> ::= '(' <Expression> ')'
        [Value_Num] = 104                         ' <Value> ::= NUM
        [Value_Float] = 105                       ' <Value> ::= FLOAT
        [Value_Str] = 106                         ' <Value> ::= STR
        [Value_True] = 107                        ' <Value> ::= TRUE
        [Value_False] = 108                       ' <Value> ::= FALSE
        [Paramlist_Comma] = 109                   ' <Param List> ::= <Expression> ',' <nl Opt> <Param List>
        [Paramlist] = 110                         ' <Param List> ::= <Expression>
        [Paramlist2] = 111                        ' <Param List> ::= 
    End Enum
        ...
        With Reduction
            Select Case .Parent.TableIndex
                ...
                Case ProductionIndex.Header2
                    ' <Header> ::= <DefineList> 
                    Return New Header(Nothing, .Item(0).Data)
                ...
                Case ProductionIndex.Definelist3
                    ' <DefineList> ::=  
                ...
                Case ProductionIndex.Methodcall_Dibujarast_Lparen_Id_Rparen_Semi
                    ' <Method Call> ::= DIBUJARAST '(' ID ')' ';' <nl Opt> 
                    Return New DibujarAST(CStr(.Item(2).Data))

                Case ProductionIndex.Methodcall_Dibujarexp_Lparen_Rparen_Semi
                    ' <Method Call> ::= DIBUJAREXP '(' <Expression> ')' ';' <nl Opt> 
                    Return New DibujarEXP(.Item(2).Data)
